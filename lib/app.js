$(document).ready(function(){
	...
	$('.btnThem').click(function(e){
		...
		$.ajax({
			...
			success: function(res){
				...
				if(err == 0)
					$('body').append('<div class="alert alert-success alert-top fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>Thông báo!</strong> Bạn đã thêm xe vào giỏ hàng thành công</div>');
				else
					$('body').append('<div class="alert alert-danger alert-top fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>Lỗi!</strong> Bạn đã thêm xe vào giỏ hàng không thành công</div>');
			...
			success: function(res)
			{
				...
				if(err == 0)
				{
					...
					else
					{
						$("table").remove();
						$(".panel-footer").remove();
						$(".panel-body").html("<h5>Bạn không có xe nào trong giỏ</h5><div><a href='index.php'>Nhấp vào đây để xem các xe khác</a></div>");
					}
	...
	$('#comments-container').comments({
		...
		{
			$.ajax({
				url: 'Controller/Xe/BinhLuan.php',
				type: 'get',
