	...
	<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 aside" >
		<form role="form" id="frmTimKiem">
			<div class="form-group">
				<label for="txtTenSach">Tên xe: </label>
				<input type="text" class="form-control" name="tensach" id="txtTenSach" placeholder="Nhập tên xe cần tìm" <?php if(!empty($_GET['tensach'])) echo "value='{$_GET['tensach']}'"; ?>>
			</div>
			<div class="form-group">
				<label for="txtTenTG">Tên nhà phân phối: </label>
				<input type="text" class="form-control" name="tentg" id="txtTenTG" placeholder="Nhập tên nhà phân phối cần tìm" <?php if(!empty($_GET['tentg'])) echo "value='{$_GET['tentg']}'"; ?>>
			</div>
			<div class="form-group">
				<label for="madms">Hãng xe: </label>
...
<?php
	if(count($result) == 0)
	{
		echo "<h5>Không có xe. Bạn hãy tìm kiếm với từ khoá khác.</h5>";
	}
	else
	{
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					<div class="panel panel-default panel-sach">
						<div class="panel-body">
							...
							<div class="col-sm-8">
								<div class="tensach"><a href="index.php?c=Xe&act=ChiTiet&madms={$row['MADMS']}&maloai={$row['MALOAI']}&masach={$row['MAXE']}">{$row['TENSACH']}</a></div>
								<div class="tentg">Nhà phân phối: {$row['TENTG']}</div>
								<div class="tennxb">Nhà phân phối: {$row['TENNXB']}</div>
