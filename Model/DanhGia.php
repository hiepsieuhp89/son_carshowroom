<?php
	...
	class DanhGia
	{
		...
		{
			$sql = "SELECT SUM(DIEM) AS TONGDIEM, COUNT(*) AS SL FROM DANHGIA WHERE MAXE = $masach";
			return $this->cn->Fetch($sql);
		}

		function LayDanhGiaKH($masach, $makh)
		{
			$sql = "SELECT DIEM FROM DANHGIA WHERE MAXE = $masach AND MAKH = $makh";
			return $this->cn->Fetch($sql);
		}
		...
		{
			$sql = "UPDATE DANHGIA SET DIEM = $diem WHERE MAXE = $masach AND MAKH = $makh";
			return $this->cn->ExecuteQuery($sql);
		}

		function XoaDanhGia($masach, $makh)
		{
			$sql = "DELETE FROM DANHGIA WHERE MAXE = $masach AND MAKH = $makh";
			return $this->cn->ExecuteQuery($sql);
		}
